...
export const tools: ToolDefinition[] = [
  tool('regex-tester', 'development', 'REGEX', 'local', 'Regex tester and replacer', 'Test regular expressions, inspect matches and preview replacements locally in your browser.', 'Probador y reemplazador de regex', 'Prueba expresiones regulares, revisa coincidencias y previsualiza reemplazos localmente.', { search: 'regex regexp regular expression tester replace pattern flags expresion regular probar reemplazar patron' }),
  tool('hash-generator', 'development', 'HASH', 'local', 'SHA hash generator', 'Calculate SHA-1, SHA-256, SHA-384 and SHA-512 hashes for text or files with Web Crypto.', 'Generador de hashes SHA', 'Calcula hashes SHA-1, SHA-256, SHA-384 y SHA-512 de textos o archivos con Web Crypto.', { search: 'hash sha digest checksum file integrity checksum suma verificacion archivo crypto' }),
  tool('unit-converter', 'finance', 'UNIT', 'local', 'Unit converter', 'Convert length, mass, temperature and digital storage units instantly and offline.', 'Conversor de unidades', 'Convierte unidades de longitud, masa, temperatura y almacenamiento al instante y sin conexión.', { search: 'unit converter length mass temperature data units conversor unidades longitud peso temperatura datos' }),
  ...
  tool('web-icon-generator', 'images', 'WEB', 'local', 'Web icon generator', 'Turn one image into favicons and web icons with visual position and zoom controls.', 'Generador de iconos para web', 'Convierte una imagen en favicons e iconos web ajustando visualmente posición y zoom.', { search: 'favicon web icon iconos web apple touch android chrome manifest zoom posicion imagen' }),
  ...
  tool('diff-checker', 'development', 'DIFF', 'local', 'Visual text diff checker', 'Compare two texts locally with line and word-level visual differences.', 'Comparador visual de textos', 'Compara dos textos localmente con diferencias visuales por línea y palabra.', { search: 'diff checker compare text comparar textos visual lcs patch line word character' }),
  ...
  tool('text-case-converter', 'writing', 'CASE', 'local', 'Text case converter', 'Convert text to uppercase, lowercase, title case, sentence case, camelCase or snake_case.', 'Conversor de mayúsculas y minúsculas', 'Convierte texto a mayúsculas, minúsculas, título, oración, camelCase o snake_case.', { search: 'case uppercase lowercase title camel snake mayusculas minusculas texto' }),
  tool('slug-generator', 'writing', 'SLUG', 'local', 'SEO slug generator', 'Turn titles into clean, readable and search-friendly URL slugs.', 'Generador de slugs SEO', 'Convierte títulos en slugs de URL limpios, legibles y preparados para SEO.', { search: 'slug seo url permalink title titulo ruta normalizar' }),
  tool('list-cleaner', 'writing', 'LIST', 'local', 'List cleaner and sorter', 'Trim, deduplicate, sort and remove empty lines from pasted lists.', 'Limpiador y ordenador de listas', 'Recorta, elimina duplicados, ordena y borra líneas vacías de listas.', { search: 'list sort deduplicate unique clean lista ordenar duplicados limpiar' }),
  tool('html-entities', 'development', '&;', 'local', 'HTML entity encoder and decoder', 'Encode reserved HTML characters or decode entities locally.', 'Codificador y decodificador de entidades HTML', 'Codifica caracteres reservados de HTML o decodifica entidades localmente.', { search: 'html entities encode decode escape entidades codificar decodificar escapar' }),
  tool('json-to-typescript', 'development', 'TS', 'local', 'JSON to TypeScript converter', 'Generate readable TypeScript interfaces from JSON objects and arrays.', 'Conversor de JSON a TypeScript', 'Genera interfaces TypeScript legibles desde objetos y arrays JSON.', { search: 'json typescript interface type api converter convertir tipos interfaz' }),
  tool('number-base-converter', 'development', 'BASE', 'local', 'Number base converter', 'Convert integers between binary, octal, decimal and hexadecimal.', 'Conversor de bases numéricas', 'Convierte enteros entre binario, octal, decimal y hexadecimal.', { search: 'number base binary octal decimal hexadecimal bigint base numeros binario' }),
...
export const getLocalizedTools = (lang: Lang): LocalizedTool[] => tools.map((item) => ({ ...item, title: item.text[lang].title, description: item.text[lang].description, categoryLabel: toolCategoryLabels[item.category][lang], privacyLabel: privacyLabels[item.privacy][lang], path: `${lang === 'es' ? '/es' : ''}/${item.slug}/` }));
